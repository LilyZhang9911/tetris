...
const int rotation[4][4][2] = {{{1, -1}, {-2, 0}, {-1, -1}, {0, -2}},
                                {{0, -1}, {1, 2}, {0, 1}, {-1, 0}},
                                {{-2, 0}, {1, -1}, {0, 0}, {-1, 1}},
                                {{1,2}, {0, -1}, {1, 0}, {2, 1}}};

L::L (int level, Board *parent) : Block{level, parent, "L"}, pos {0} {
    //set to top left beginning position
    cells[0] = parent->getCell(3,2); //store cell as one of its cells
    parent->getCell(3,2)->turnOn (type); //turn on cell

    cells[1] = parent->getCell(4,0);
    parent->getCell(4,0)->turnOn (type);

    cells[2] = parent->getCell(4,1);
    parent->getCell(4,1)->turnOn (type);

    cells[3] = parent->getCell(4,2);
    parent->getCell(4,2)->turnOn (type);
